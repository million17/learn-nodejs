...
const composeAsync = (...fns) => arg => {
    return _.flattenDeep(fns).reduceRight(async (current, fn) => {
        ...
        throw new TypeError('Compose() expects only function as parameters');
    }, arg);
};
...
const enforceHttpsUrl = url =>
    _.isString(url) ? url.replace(/^(http:?:)?\/\//, "https://") : null;
...
const sanitizeNumber = number =>
    _.isString(number) ? number.replace(/[^0-9-.]/g, "")
 ...
 */
const extractFromElems = extractor => transform => elem => $ => {
    const rs = elem.map((i, e) => extractor($(e))).get();
    return _.isFunction(transform) ? transform(rs) : rs;
}
